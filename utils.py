...
def load_val_dataset(args, val_dataset_name):
    ...
    if val_dataset_name == 'cifar10':
        ...
        val_dataset = eurosat.EuroSAT(args.root, transform=preprocess224, download=False)
        ...
        val_dataset = flowers102.Flowers102(args.root, split='test', transform=preprocess224, download=False)
...
def get_text_prompts_val(val_dataset_list, val_dataset_name, template='This is a photo of a {}.', force_template=False):
    ...
    for cnt, each in enumerate(val_dataset_list):
        if hasattr(each, 'clip_prompts') and not force_template:
            ...
            if hasattr(each, 'classes'):
                class_names = each.classes
            elif hasattr(each, '_classes'):
                class_names = each._classes
            elif hasattr(each, 'clip_categories'):
                class_names = each.clip_categories
            else:
                class_names = [each.idx_to_class[i] for i in range(len(each.idx_to_class))]
            if val_dataset_name[cnt] in ['ImageNet', 'tinyImageNet']:
                import os as _uos
                support_dir = _uos.path.join(_uos.path.dirname(_uos.path.abspath(__file__)), '..', 'support')
                refined_file = _uos.path.join(support_dir, f"{val_dataset_name[cnt].lower()}_refined_labels.json")
                try:
                    refined_data = read_json(refined_file)
                    clean_class_names = [refined_data[ssid]['clean_name'] for ssid in class_names]
                    class_names = clean_class_names
                except (KeyError, FileNotFoundError):
                    pass  # Use class names as-is (numeric labels OK for ImageNet-100)
